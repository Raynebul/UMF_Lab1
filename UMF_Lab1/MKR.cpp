...
{
	type delta = 1000;  // относительная невязка
...
void MKR::Grid()
{
	...
	vector<Area> vectorN;
	type X, Y; //точки x и y, коэффициент разрядки?
	...
	for (int currCountY = 0; currCountY < countY - 1; )
	{
		...
		{
			hy = (Y - vectorY[currCountY]) * (ky[currCountY] - 1) / (pow(ky[currCountY], Ny + 1) - 1); //геометричесаяпрогрессия (1+ kx + k2x + k3x...)
